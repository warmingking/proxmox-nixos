...
def main():
    ...
    repo_url = args.url if args.url else f'https://github.com/proxmox/{pkg_name}.git'
            ...
            def transform_git_deps():
                ...
                for dep in deps:
                    ...
                    if dep == 'perlmod':
                        ...
                        run_command(
                            f'toml set Cargo.toml dependencies.{dep}.git https://github.com/proxmox/proxmox-backup.git > Cargo.toml.tmp && mv Cargo.toml.tmp Cargo.toml')
